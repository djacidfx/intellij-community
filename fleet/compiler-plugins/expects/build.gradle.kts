...
val pluginVersion = run {
  ...
  project.file("src/main/kotlin/fleet/multiplatform/expects/gradle/ExpectsGradlePlugin.kt").readLines().firstNotNullOfOrNull { line ->
    versionRegex.find(line)?.let { match ->
      match.groupValues[1]
    }
  } ?: error("Cannot find `const val VERSION = \"...\"` in ExpectsGradlePlugin.kt")
}
...
gradlePlugin {
  plugins.register("expectsCompilerGradlePlugin") {
    id = "fleet-multiplatform-expects-module"
    implementationClass = "fleet.multiplatform.expects.gradle.ExpectsGradlePlugin"
  }
}
